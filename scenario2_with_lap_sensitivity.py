...
data = {
    ...
    "Laptime (s)": [95.47, 91.8, 81.69, 106.4, 91.46, 79.33, 76.15, 85.61, 75.9, 67.81, 91, 82.1, 110.6, 73.68, 84.45,
                    106.46, 105, 85.04, 99.67, 81.87, 74.21, 94.5, 89.07],
    "Lap Distance (m)": [5356, 6098, 5229, 5948, 5337, 4870, 3281, 4635, 4312, 4289, 5812, 4343, 6933, 4236, 5757, 5004,
                         5810, 5384, 5444, 4244, 4250, 6201, 5222],
    "Number of Race Laps": [57, 50, 58, 51, 57, 63, 79, 66, 71, 71, 52, 70, 44, 72, 53, 61, 52, 57, 56, 72, 72, 49, 58],
    "Race Laptime Sensitivity (ms/kW per lap)": [22, 29, 29, 29, 21, 19, 10, 15, 18, 16, 30, 13, 33, 18, 33, 12, 28, 21,
                                                 25, 13, 19, 27, 24]
}
...
# Parameters for the new scenario
...
PUs_PER_SEASON = 5  # Increased due to need for PU5

# Modifying engine details for the new scenario, here you can add or recall engines,
# as well as define when they are available, added "recall_race" for engine failure.
engines = {
    1: {"available_race": 1, "current_km": 0, "degradation": 0, "recall_race": 100},
    2: {"available_race": 1, "current_km": 0, "degradation": 0, "recall_race": 8},
    3: {"available_race": 9, "current_km": 0, "degradation": 0, "recall_race": 100},
    4: {"available_race": 13, "current_km": 0, "degradation": 0, "recall_race": 100},
    5: {"available_race": 14, "current_km": 0, "degradation": 0, "recall_race": 100}
}


# Function to find the best engine for a given race, added recall_race to suitable_engine condition
def find_best_engine(round_number, lap_sensitivity):
    suitable_engines = [key for key, value in engines.items() if
                        value["available_race"] <= round_number <= value["recall_race"] and value[
...
# Allocating engines to races for the new scenario
...
print(df)
df.to_csv("lap_sensitivity_race_strategy_scenario_2.csv", index=False)
